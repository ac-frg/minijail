...
/*
 * A hook that can be used to install LSM mechanisms at various events during
 * minijail setup. These can only be used if the jailed process is not going to
 * be invoked with LD_PRELOAD.
 *
 * If the return value is non-zero, it will be interpreted as -errno and the
 * process will abort.
 */
typedef int (*minijail_lsm_hook_t)(void *context);
...
typedef enum {
	...
	MINIJAIL_LSM_HOOK_EVENT_PRE_DROP_CAPS,

	/* The hook will run just before calling execve(2). */
	MINIJAIL_LSM_HOOK_EVENT_PRE_EXECVE,

	/* Sentinel for error checking. Must be last. */
	MINIJAIL_LSM_HOOK_EVENT_MAX,
} minijail_lsm_hook_event_t;
...
/*
 * minijail_add_lsm_hook: adds @hook to the list of hooks that will be
 ...
 */
int minijail_add_lsm_hook(struct minijail *j,
			  minijail_lsm_hook_t hook, void *payload,
