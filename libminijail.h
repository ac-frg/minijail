 ...
 * call needs to be allowed. Otherwise the child process will be attempt to
 * send the file descriptor to the parent, and have this call blocked by the
 * kernel since it will wait for the parent to receive the notification. The
 * parent won't be ever able to receive the notification file descriptor at
 * all, so both processes will deadlock.
 */
