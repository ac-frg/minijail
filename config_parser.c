...
static void insert_entry(struct config_entry *head, struct config_entry *entry)
{
	if (!head || !entry) {
		return;
	}
...
static void free_config_entry(struct config_entry *entry) {
	if (entry->key) {
		free(entry->key);
	}
	if (entry->value) {
