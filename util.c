...
{
	if (g_logging_config.logger == LOG_TO_SYSLOG) {
		va_list args;
	...
	if (g_logging_config.min_priority < priority)
		return;
	...
	while (buffer_length) {
		written = write(g_logging_config.fd, pos, buffer_length);
		if (written <= 0)
			return;
		pos += written;
...
{
	g_logging_config.logger = logger;
	g_logging_config.fd = fd;
	g_logging_config.min_priority = min_priority;
}
