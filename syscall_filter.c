...
bool check_duplicate_syscalls(void)
{
#if defined(CHECK_DUP_SYSCALLS)
	return true;
#endif
	return false;
}
		 ...
		 unsigned int include_level)
{
	...
	while (getmultiline(&line, &len, policy_file) != -1) {
		...
		if (filteropts->check_duplicate_syscalls &&
		    !insert_and_check_duplicate_syscall(previous_syscalls,
							&state, ind)) {
			ret = -1;
		   ...
		   const struct filter_options *filteropts)
{
	 ...
	 */
	struct parser_state **previous_syscalls = NULL;
	if (filteropts->check_duplicate_syscalls) {
