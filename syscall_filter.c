 ...
 * index ind if it is a newly encountered syscall. Returns false if it is a
 * newly encountered syscall and true if it is a duplicate.
 */
		 ...
		 unsigned int include_level)
{
	...
	while (getmultiline(&line, &len, policy_file) != -1) {
		...
		/*
		 * ind can be cast to a size_t because it is positive as
		 * confirmed by the earlier check.
		 */
