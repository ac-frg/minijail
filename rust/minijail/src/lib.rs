...
impl Minijail {
    ...
    // This is safe because it does not modify the struct and
    pub fn wait(&self) -> Result<()> {
        ...
        if ret > MINIJAIL_ERR_SIG_BASE as libc::c_int {
...
mod tests {
    ...
    fn wait_noaccess() {
        let j = Minijail::new().unwrap();
        j.run(Path::new("/dev/null"), &[1, 2], &["/dev/null"])
            .unwrap();
