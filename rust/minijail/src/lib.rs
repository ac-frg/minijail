...
impl Minijail {
    ...
    pub unsafe fn fork(&self, inheritable_fds: Option<&[RawFd]>) -> Result<pid_t> {
        match inheritable_fds {
            Some(fds) => {
                let m: Vec<(RawFd, RawFd)> = fds.iter().map(|&a| (a, a)).collect();
                self.fork_remap(Some(&m))
            }
            None => self.fork_remap(None),
        }
    }

    /// Behaves the same as `fork()` except `inheritable_fds` is a list of fd
    /// mappings rather than just a list of fds to preserve.
    pub unsafe fn fork_remap(&self, inheritable_fds: Option<&[(RawFd, RawFd)]>) -> Result<pid_t> {
        ...
        for io_fd in &[libc::STDIN_FILENO, libc::STDOUT_FILENO, libc::STDERR_FILENO] {
            if inheritable_fds.is_none()
                || !inheritable_fds.unwrap().iter().any(|(_, fd)| *fd == *io_fd)
            {
