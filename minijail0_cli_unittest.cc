...
// Valid calls to the add-supp-group option.
TEST_F(CliTest, valid_add_supp_group) {
  std::vector<std::string> argv = {"--add-supp-group", "", "/bin/sh"};
  ...
  std::vector<std::string> argv2 = {"--add-supp-group", "",
                                    "--add-supp-group", "", "/bin/sh"};
  argv[1] = kValidGroup;
  argv[2] = kValidGid;
  ASSERT_TRUE(parse_args_(argv));
}

// Invalid calls to the add-supp-group option.
