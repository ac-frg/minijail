...
void HandleSeccompUserNotificationRequest(int notify_fd,
                                          int expected_syscall_number,
                                          pid_t fake_tid) {
  // Normally this is obtained by calling seccomp(SECCOMP_GET_NOTIF_SIZES,
  // 0, &sizes), but that causes some linker errors in the test. Instead,
  // hardcode a buffer size that's large enough to hold any possible
  // buffer.
  constexpr size_t kNotificationBufferSizes = 1024;

  uint8_t req_buf[kNotificationBufferSizes];
  struct seccomp_notif *req =
      reinterpret_cast<struct seccomp_notif *>(req_buf);
  ...
  EXPECT_EQ(req->data.nr, expected_syscall_number);

  uint8_t resp_buf[kNotificationBufferSizes];
  struct seccomp_notif_resp *resp =
      reinterpret_cast<struct seccomp_notif_resp *>(resp_buf);
...
void MinijailEnterHelper() {
  ...
  minijail_seccomp_install_user_notification(j.get());
  ...
  EXPECT_EQ(minijail_seccomp_user_notification_fd(j.get()), -1);
  ...
  int notify_fd = minijail_seccomp_user_notification_fd(j.get());
  ASSERT_NE(notify_fd, -1);
  EXPECT_EQ(minijail_seccomp_user_notification_fd(j.get()), -1);

  constexpr pid_t kFakeTid = 0xDEADBEEF;
  HandleSeccompUserNotificationRequest(notify_fd, __NR_gettid, kFakeTid);
...
TEST(UserNotificationTest, minijail_enter) {
  ...
  // We need to run the test in another process to avoid having this process
  // permanently installed with a seccomp filter, which wil cause any future
  // tests to fail.
  ...
  int status = -1;
  EXPECT_EQ(waitpid(child_pid, &status, 0), child_pid);
  ASSERT_TRUE(WIFEXITED(status));
  EXPECT_EQ(WEXITSTATUS(status), 0);
}

TEST(UserNotificationTest, minijail_run_pid_pipes) {
  // libminijail does not allow setting seccomp filters with (HW)ASan enabled.
  if (!seccomp_filter_flags_available(SECCOMP_FILTER_FLAG_NEW_LISTENER) ||
      running_with_asan()) {
    SUCCEED();
  ...
  int child_stdout;
  char *const argv[] = {"sh", "-c", "echo $$", nullptr};
  EXPECT_EQ(minijail_run_pid_pipes(j.get(), kShellPath, argv, nullptr, nullptr,
  ...
  constexpr pid_t kFakePid = 123456789;
  HandleSeccompUserNotificationRequest(notify_fd, __NR_getpid, kFakePid);

  char buf[kBufferSize] = {};
  EXPECT_GT(read(child_stdout, buf, sizeof(buf) - 1), 0);
