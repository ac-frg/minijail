 ...
 * |*conf_entry_list|, pointed. |optarg| is updated to point to the entry value
 * as the option argument. If all options have been consumed, |*conf_entry_list|
 * will be freed.
 */
			    ...
			    size_t *conf_index)
{
	...
	struct config_entry *entry =
		    &(*conf_entry_list)->entries[*conf_index];
	size_t opt_index = 0;
	bool matched = false;
	/* look up a matching long option. */
	while (longopts[opt_index].name != NULL) {
		if (strcmp(entry->key, longopts[opt_index].name) == 0) {
			opt = longopts[opt_index].val;
						...
						size_t *conf_index)
{
	...
	if (*conf_entry_list != NULL)
		opt = getopt_from_conf(long_options, conf_entry_list, conf_index);
	if (opt == -1)
	       ...
	       const char **preload_path)
{
	...
	while ((opt = getopt_conf_or_cli(argc, argv, &conf_entry_list,
		...
		case 135:
			...
			break;
		case 136: {
			/* '--config', when present, should be the first option.
			...
			if (conf_entry_list != NULL) {
				printf("Nested config file specification is not allowed.\n");
				exit(1);
			}
			conf_entry_list = new_config_entry_list();
			conf_index = 0;
			FILE *config_file = fopen(optarg, "re");
			if (!config_file) {
				fprintf(stderr, "failed to open %s: %m", optarg);
				exit(1);
			}
