...
static void parse_conf_file(const char *conf_path)
{
	...
	while (fgets(line, CONF_LINE_MAX_SIZE, f) != NULL) {
		 ...
		 * It is inefficient to do string comparisons one-by-one but
		 * the conf file is expected to be small. Keyword parsing can
		 * be later improved by a trie or a hash lookup.
		 */
		if (strcmp(key, "execute_paths") == 0) {
			/* TODO: process the value as a list of comma separated paths.
			 * The paths will be added to a Landlock ruleset as read and execute
			 * paths.
			 */
		} else if (strcmp(key, "read_only_paths") == 0) {
			/* TODO: process the value as a list of comma separated paths.
			 * The paths will be added to a Landlock ruleset as read-only
			 * paths.
			 */
		} else if (strcmp(key, "write_paths") == 0) {
			/* TODO: process the value as a list of comma separated paths.
			 * The paths will be added to a Landlock ruleset as read-write
			 * paths.
			 */
		} else if (strcmp(key, "binding") == 0) {
			/* TODO: process the value as '-b' option, i.e. add_binding(j, value); */
		} else if (strcmp(key, "mount") == 0) {
			/* TODO: process the value as '-m' option, i.e.e add_mount(j, value); */
		} else {
