			    ...
			    size_t *conf_index)
{
	...
	size_t i = 0;
	const struct option *curr_opt = &longopts[0];
	bool matched = false;
	for (; curr_opt->name != NULL; curr_opt = &longopts[++i]) {
		if (strcmp(entry->key, curr_opt->name) == 0) {
			opt = curr_opt->val;
			matched = true;
			switch (curr_opt->has_arg) {
			case required_argument:
				optarg = (char *)entry->value;
				break;
			case optional_argument:
				/* Literal true translates to empty argument. */
				if (strcmp(entry->value, literal_true) == 0)
					optarg = NULL;
				else
					optarg = (char *)entry->value;
				break;
			case no_argument:
				/* Only accept literal true. */
				if (strcmp(entry->value, literal_true) == 0)
					optarg = NULL;
				else {
					fprintf(
					    stderr,
					    "In a config file, 'true' is the "
					    "only accepted value for '%s'",
					    entry->key);
					exit(1);
				}
				break;
			default:
				fprintf(
				    stderr,
				    "Encountered a bad struct option value.");
				exit(1);
			}
			break;
		}
