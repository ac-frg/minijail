	       ...
	       const char **preload_path)
{
	...
	while ((opt = getopt_conf_or_cli(argc, argv, &conf_entry_list,
		...
		case 136: {
			...
			* With a W^X invariant, it excluedes writable
			* mounts.
			*/
			struct statfs conf_statfs;
			if (statfs(optarg, &conf_statfs) != 0)
				errx(1, "Failed statfs() on %s: %m", optarg);
			if ((conf_statfs.f_flags & MS_NOEXEC) != 0)
				errx(1,
				     "Conf file should be in a exec "
				     "mount: %s\n",
				     optarg);
#endif
			...
			if (stat("/", &root_stat) != 0)
				errx(1, "Failed stat() on rootfs: %m");
			if (stat(optarg, &conf_stat) != 0)
				errx(1, "Failed stat() on %s: %m", optarg);
			if (root_stat.st_dev != conf_stat.st_dev)
				errx(1, "Conf file should locate in the same device of the rootfs.");
#endif
