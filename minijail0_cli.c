			      ...
			      size_t *conf_index)
{
	...
	/*
	 * A separate option list that helps introducing new conf options
	 * gradually and independently from the cli options.
	 */
	static const struct option conf_options[] = {
		{"mount", required_argument, 0, 'k'},
		{"bind-mount", required_argument, 0, 'b'},
		{0, 0, 0, 0},
	};
	...
	if (*conf_entry_list != NULL)
		opt =
		    getopt_from_conf(conf_options, conf_entry_list, conf_index);
	if (opt == -1)
