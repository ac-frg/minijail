...
TEST(bpf, bpf_comp_in) {
  struct sock_filter comp_in[BPF_COMP_LEN];
  unsigned long mask =
      (1UL << (sizeof(unsigned long) * 8 - 1)) | O_WRONLY;
  unsigned char jt = 10;
  unsigned char jf = 20;

  size_t len = bpf_comp_in(comp_in, mask, jt, jf);

  EXPECT_EQ(len, BPF_COMP_LEN);

#if defined(BITS32)
  EXPECT_EQ_BLOCK(&comp_in[0], BPF_JMP + BPF_JSET + BPF_K, ~mask, jf, jt);
#elif defined(BITS64)
  EXPECT_EQ_BLOCK(&comp_in[0], BPF_JMP + BPF_JSET + BPF_K, 0x7FFFFFFF,
      jf + 2, 0);
  EXPECT_EQ_STMT(&comp_in[1], BPF_LD + BPF_MEM, 0);
  EXPECT_EQ_BLOCK(&comp_in[2], BPF_JMP + BPF_JSET + BPF_K, ~O_WRONLY, jf,
      jt);
#endif
}
