...
TEST_F(ArgFilterTest, arg0_short_gt_ge_comparisons)
{
	for (std::string fragment :
		...
		struct filter_block *block = test_compile_policy_line(
		    &state_, nr_, fragment, id_, &labels_);

		ASSERT_NE(block, nullptr);
		size_t exp_total_len =
		    1 + (BPF_ARG_SHORT_GT_GE_COMP_LEN + 1) + 2 + 1 + 2;
		EXPECT_EQ(block->total_len, exp_total_len);
...
TEST_F(ArgFilterTest, arg0_long_gt_ge_comparisons)
{
	for (std::string fragment :
		...
		ASSERT_NE(block, nullptr);
		size_t exp_total_len =
		    1 + (BPF_ARG_GT_GE_COMP_LEN + 1) + 2 + 1 + 2;
		EXPECT_EQ(block->total_len, exp_total_len);
...
TEST_F(FileTest, repeat_syscall)
{
...
TEST_F(FileTest, seccomp_read)
{
	...
	/*
	 * Checks return value, that the blocks only allow expected syscalls,
	 * and that labels between |head_| and |arg_blocks_| match.
	 */
	ASSERT_EQ(res, 0);
	struct filter_block *curr_block = head_;
	ASSERT_NE(curr_block, nullptr);
	EXPECT_ALLOW_SYSCALL_ARGS(curr_block->instrs, __NR_read, LABEL_ID,
				  JUMP_JT, JUMP_JF);
	curr_block = curr_block->next;
	ASSERT_NE(curr_block, nullptr);
	EXPECT_ALLOW_SYSCALL(curr_block->instrs, __NR_write);
...
TEST_F(FileTest, multiline)
{
	...
	curr_block = curr_block->next;
	ASSERT_NE(curr_block, nullptr);
	EXPECT_ALLOW_SYSCALL_ARGS(curr_block->instrs, __NR_openat, LABEL_ID,
				  JUMP_JT, JUMP_JF);
...
TEST(FilterTest, seccomp_mode1_log)
{
	...
	int res = test_compile_filter("policy", policy_file, &actual,
				      ACTION_RET_LOG, USE_RET_LOG_LOGGING);
	fclose(policy_file);
...
TEST(FilterTest, log)
{
	...
	int res = test_compile_filter("policy", policy_file, &actual,
				      ACTION_RET_TRAP, USE_SIGSYS_LOGGING);
	fclose(policy_file);
...
TEST(FilterTest, allow_log_but_kill)
{
	...
	int res = test_compile_filter("policy", policy_file, &actual,
				      ACTION_RET_KILL, USE_SIGSYS_LOGGING);
	fclose(policy_file);
...
TEST(FilterTest, include)
{
	...
	std::string policy_plain = "read: 1\n"
				   "write: 1\n"
				   "rt_sigreturn: 1\n"
				   "exit: 1\n";
	...
	int res_plain = test_compile_filter("policy", file_plain,
					    &compiled_plain, ACTION_RET_KILL);
	fclose(file_plain);
	...
	int res_with_include =
	    test_compile_filter("policy", file_with_include,
				&compiled_with_include, ACTION_RET_KILL);
	fclose(file_with_include);

	/*
	 * Checks that filter length is the same for a plain policy and an
	 * equivalent policy with an @include statement. Also checks that the
	 * filter generated from the policy with an @include statement is
	 * exactly the same as one generated from a plain policy.
	 */
	...
	EXPECT_EQ_STMT(compiled_with_include.filter + ARCH_VALIDATION_LEN,
		       BPF_LD + BPF_W + BPF_ABS, syscall_nr);
	EXPECT_ALLOW_SYSCALL(
	    compiled_with_include.filter + ARCH_VALIDATION_LEN + 1, __NR_read);
	EXPECT_ALLOW_SYSCALL(
	    compiled_with_include.filter + ARCH_VALIDATION_LEN + 3, __NR_write);
	EXPECT_ALLOW_SYSCALL(compiled_with_include.filter +
				 ARCH_VALIDATION_LEN + 5,
			     __NR_rt_sigreturn);
	EXPECT_ALLOW_SYSCALL(
	    compiled_with_include.filter + ARCH_VALIDATION_LEN + 7, __NR_exit);
...
TEST(FilterTest, include_nested)
{
	...
	FILE *included_file =
	    fopen(source_path("test/nested.policy").c_str(), "re");
	ASSERT_NE(included_file, nullptr);
	fclose(included_file);
