...
static void usage(const char *progn)
{
	...
	printf(
	    "Usage: %s [-GhHiIKlLnNprstUvY]\n"
	    "  [-a <table>]\n"
	    "  [-b <src>,<dest>[,<writeable>]]\n"
	    "  [-k <src>,<dest>,<type>[,<flags>][,<data>]]\n"
	    "  [-c <caps>] [-C <dir>] [-P <dir>] [-e[file]] [-f <file>]\n"
	    "  [-g <group>] [-m[<uid> <loweruid> <count>]*]\n"
	    ...
	    "              <flags> and <data> can be specified as in\n"
	    "              mount(2).\n"
	    "              Multiple instances allowed.\n"
	    ...
	    "  -e[file]:   Enter new network namespace, or existing one if\n"
	    "              |file| is provided.\n"
	    ...
	    "              Not compatible with -p.\n"
	    "  -I:         Run <program> as init (pid 1) inside a new pid\n"
	    "              namespace (implies -p).\n"
	    "  -K:         Don't mark all existing mounts as MS_PRIVATE.\n"
	    "  -l:         Enter new IPC namespace.\n"
	    "  -L:         Report blocked syscalls to syslog when using\n"
	    "              seccomp filter.\n"
	...
	printf(
	    ...
	    "              Same arguments as newuidmap(1), multiple mappings\n"
	    "              should be separated by ',' (comma).\n"
	    "              Not compatible with -b without the 'writable'\n"
	    "              option.\n"
	    "  -M:         Set the gid map of a user namespace (implies -pU).\n"
	    "              Same arguments as newgidmap(1), multiple mappings\n"
	    "              should be separated by ',' (comma).\n"
	    "              Not compatible with -b without the 'writable'\n"
	    "              option.\n"
	    "  -n:         Set no_new_privs.\n"
	    ...
	    "  -T <type>:  Don't access <program> before execve(2), assume\n"
	    "              <type> ELF binary.\n"
