...
static void drop_securebits(struct minijail *j, const char *arg)
{
	uint64_t securebits_drop_mask;
	char *end = NULL;
	securebits_drop_mask = strtoull(arg, &end, 16);
	if (*end) {
		fprintf(stderr, "Invalid securebit mask: '%s'\n", arg);
		exit(1);
	}
	minijail_drop_securebits(j, securebits_drop_mask);
}
...
static void usage(const char *progn)
{
	...
	printf("Usage: %s [-GhHiIKlLnNprstUvyYz]\n"
	       ...
	       "  -B <mask>     Drops <mask> from the securebits that are set when restricting caps\n"
	       "                (-c). By default, SECURE_NOROOT, SECURE_NO_SETUID_FIXUP, and \n"
		      ...
		      int *exit_immediately, ElfType *elftype)
{
	...
	const char *optstring =
	    "+u:g:sS:c:C:P:b:B:V:f:m::M::k:a:e::T:vrGhHinNplLt::IUKwyYz";
