...
char API *minijail_get_original_path(struct minijail *j, const char *chroot_path)
{
	...
	while (b) {
		/*
		 * If the chroot path _is_ the bind mount point, return the
		 * external path.
		 */
		if (!strcmp(b->dest, chroot_path))
			return strdup(b->src);

		/*
		 * If the chroot path is within a bind mount, use that path
		 * concatenated with the relative path from the bind mount
		 * destination to the chroot path.
		 */
