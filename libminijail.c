...
void minijail_preexec(struct minijail *j)
{
	...
	int keep_session_keyring = j->flags.keep_session_keyring;
	if (j->user)
		free(j->user);
	j->user = NULL;
	if (j->suppl_gid_list)
	...
	j->flags.keep_session_keyring = keep_session_keyring;
...
void API minijail_keep_session_keyring(struct minijail *j)
{
	j->flags.keep_session_keyring = 1;
}
...
void API minijail_enter(const struct minijail *j)
{
	...
	if (j->flags.keep_session_keyring == 0) {
		if (syscall(SYS_keyctl, KEYCTL_JOIN_SESSION_KEYRING, NULL) < 0) {
			pdie("syscall(SYS_keyctl, KEYCTL_JOIN_SESSION_KEYRING,"
			     " NULL) failed");
		}
	}
