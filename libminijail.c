...
struct lsm_hook {
	minijail_lsm_hook_t hook;
	void *payload;
	minijail_lsm_hook_event_t event;
...
struct minijail {
	...
	struct lsm_hook *lsm_hooks_head;
	struct lsm_hook *lsm_hooks_tail;
};
...
int API minijail_add_lsm_hook(struct minijail *j, minijail_lsm_hook_t hook,
			      void *payload, minijail_lsm_hook_event_t event)
{
	struct lsm_hook *c;

	if (hook == NULL)
		return -EINVAL;
	if (event >= MINIJAIL_LSM_HOOK_EVENT_MAX)
		return -EINVAL;
	c = calloc(1, sizeof(*c));
	if (!c)
		return -ENOMEM;
...
int minijail_unmarshal(struct minijail *j, char *serialized, size_t length)
{
	...
	j->mounts_tail = NULL;
	j->filter_prog = NULL;
	j->lsm_hooks_head = NULL;
	j->lsm_hooks_tail = NULL;
...
static void run_lsm_hooks_or_die(const struct minijail *j,
				 minijail_lsm_hook_event_t event)
{
	int rc;
	for (struct lsm_hook *c = j->lsm_hooks_head; c; c = c->next) {
		if (c->event != event)
			continue;
		rc = c->hook(c->payload);
		if (rc != 0) {
			errno = -rc;
			pdie("lsm_hook failed");
		}
	}
}
...
void API minijail_enter(const struct minijail *j)
{
	...
	run_lsm_hooks_or_die(j, MINIJAIL_LSM_HOOK_EVENT_PRE_DROP_CAPS);
			  ...
			  int use_preload)
{
	...
	if (use_preload && j->lsm_hooks_head != NULL) {
		die("LSM hooks are not supported with LD_PRELOAD");
	}
	...
	run_lsm_hooks_or_die(j, MINIJAIL_LSM_HOOK_EVENT_PRE_EXECVE);
...
void API minijail_destroy(struct minijail *j)
{
	...
	j->mounts_tail = NULL;
	while (j->lsm_hooks_head) {
		struct lsm_hook *c = j->lsm_hooks_head;
		j->lsm_hooks_head = c->next;
		free(c);
	}
	j->lsm_hooks_tail = NULL;
	if (j->user)
