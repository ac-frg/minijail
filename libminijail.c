				 ...
				 struct minijail_run_status *status_out)
{
	...
	/*
	 * If the parent process needs to configure the child after forking,
	 * create a pipe(2) to block the child until configuration is done.
	 */
	if (j->flags.userns || j->flags.cgroups || j->flags.pid_file ||
	    j->rlimit_count) {
		sync_child = 1;
