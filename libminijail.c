				 ...
				 struct minijail_run_state *state_out)
{
	...
	struct {
		...
		{ config->pstdin_fd, state_out->stdin_fds },
		{ config->pstdout_fd, state_out->stdout_fds },
		{ config->pstderr_fd, state_out->stderr_fds },
	};
	...
	if (sync_child)
		wait_for_parent_setup(state_out->pipe_fds);
