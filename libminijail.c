...
static void clear_seccomp_options(struct minijail *j)
{
	...
	if (j->seccomp_label) {
		free(j->seccomp_label);
	}
	j->seccomp_label = NULL;
}
				    ...
				    const struct minijail *j)
{
	...
	if (j->seccomp_label)
		marshal_append_string(state, j->seccomp_label);
}
...
int minijail_unmarshal(struct minijail *j, char *serialized, size_t length)
{
	...
	if (j->seccomp_label) {	/* stale pointer */
		char *seccomp_label = consumestr(&serialized, &length);
		if (!seccomp_label)
			goto bad_cgroups;
		j->seccomp_label = strdup(seccomp_label);
		if (!j->seccomp_label)
			goto bad_cgroups;
	}
	...
	/* If more is added after j->seccomp_label, then this is needed:
	 * if (j->seccomp_label)
	 * 	free(j->seccomp_label);
	 */
...
static int setup_seccomp_policy(const struct minijail *j, char ***child_env)
{
	return minijail_setenv(child_env, kSeccompPolicyEnvVar,
			       j->seccomp_label ? j->seccomp_label :
			       "NO-LABEL", 1);
}
				 ...
				 struct minijail_run_state *state_out)
{
	...
	if (!state_out->child_env)
		return ENOMEM;
	if (setup_seccomp_policy(j, &state_out->child_env))
		return -EFAULT;
		...
		if (WIFSIGNALED(st)) {
			...
			if (signum == SIGSYS) {
				warn("child process %d had a policy violation (%s)",
				     j->initpid,
				     j->seccomp_label ? j->seccomp_label : "NO-LABEL");
