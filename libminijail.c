...
int API minijail_add_cgroup(struct minijail *j, const char *path)
{
...
/*
 * wait_parent_setup_complete: Called by the child process to wait for any
 * further parent-side setup to complete before continuing.
 */
static void wait_parent_setup_complete(int *pipe_fds)
{
	char buf;
...
static void write_pid_to_path(int pid, const char *path)
{
...
{
	write_pid_to_path((int)j->initpid, j->pid_file_path);
}

static void assign_cgroups(const struct minijail *j, int pid)
{
	int i;

	for (i = 0; i < j->cgroup_count; i++)
		write_pid_to_path((int)j->initpid, j->cgroups[i]);
}
			  ...
			  int use_preload)
{
	...
	if (sync_child)
		wait_parent_setup_complete(child_sync_pipe_fds);
