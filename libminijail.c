				 ...
				 struct minijail_run_status *status_out)
{
	...
	if (pid_namespace) {
		...
		if (j->flags.userns)
			...
			/* The order of sys_clone arguments is platform-dependent. */
#if defined(__x86_64__)
		child_pid =
		    syscall(SYS_clone, clone_flags, NULL, NULL, NULL, 0ul);
#elif defined(__arm__) || defined(__aarch64__) || define(__i386__)
		child_pid =
		    syscall(SYS_clone, clone_flags, NULL, NULL, 0ul, NULL);
#else
#error Unsupported platform
	...
	if (child_pid) {
		...
		if (use_preload) {
			...
			while (sigtimedwait(&to_block, NULL, &zero_time) >= 0 ||
			       errno == EINTR) {
			}
