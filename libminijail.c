...
static int redirect_fds(struct minijail *j)
{
	...
	for (size_t i = 0; i < j->preserved_fd_count; i++) {
		if (j->preserved_fds[i].parent_fd ==
			...
			if (flags == -1) {
				pwarn("failed to get flags for %d",
			...
			// Currently FD_CLOEXEC is cleared without being restored. It may make sense
			// to track when this happens and restore FD_CLOEXEC in the child process.
			flags &= ~FD_CLOEXEC;
			if (fcntl(j->preserved_fds[i].child_fd, F_SETFD, flags) == -1) {
