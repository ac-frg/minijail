...
static bool seccomp_allow_logging(const struct minijail *j)
{
...
void API minijail_set_seccomp_filter_tsync(struct minijail *j)
{
	...
	if (seccomp_allow_logging(j) && !seccomp_ret_log_available()) {
		/*
				 ...
				 FILE *policy_file)
{
	...
	filteropts.allow_logging =
	    debug_logging_allowed() && seccomp_allow_logging(j);
