...
int minijail_unmarshal(struct minijail *j, char *serialized, size_t length)
{
	...
	if (j->child_ld_preload) {	/* stale pointer */
		char *ld_preload = consumestr(&serialized, &length);
		if (!ld_preload)
			goto bad_child_ld_preload;
		j->child_ld_preload = strdup(ld_preload);
		if (!j->child_ld_preload)
			goto bad_child_ld_preload;
	}
...
static int append_to_ld_preload_env(const struct minijail *j attribute_unused,
			 const char *oldenv, const char *preload_path)
{
				 ...
				 struct minijail_run_status *status_out)
{
	...
	/*
	 * We set up child_ld_preload even if !use_preload,
	 * so that potential children of the program we're about to execve()
	 * inherit the LD_PRELOAD, regardless of whether the program
	 * is dynamically or statically linked.
	 */
	if (j->child_ld_preload) {
		/*
		 * We save current LD_PRELOAD env as oldenv_copy before
		 * modifying it, so we can restore it after we've execve'd
		 * the program.
		 */
	...
	if (use_preload) {
		...
		/*
		 * LD_PRELOAD might have been modified above,
		 * get the new value.
		 */
		oldenv = getenv(kLdPreloadEnvVar);
