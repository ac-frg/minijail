...
struct minijail {
	...
	struct {
		...
		int seccomp_install_user_notification : 1;
...
void API
minijail_seccomp_install_user_notification(struct minijail *j)
{
	if (j->filter_len > 0 && j->filter_prog != NULL) {
		die("minijail_seccomp_install_user_notification() "
		    "must be called before minijail_parse_seccomp_filters()");
	}

	j->flags.seccomp_install_user_notification = 1;
}

int API minijail_seccomp_user_notification_fd(struct minijail *j)
{
...
static void clear_seccomp_options(struct minijail *j)
{
	...
	j->flags.seccomp_install_user_notification = 0;
	j->filter_len = 0;
	j->filter_prog = NULL;
...
static int seccomp_should_use_filters(struct minijail *j)
{
	...
	if (j->flags.seccomp_install_user_notification) {
...
static void set_seccomp_notification_fd(struct minijail *j, int notification_fd)
{
	if (j->flags.filter_socket_fds) {
		/*
		 * Send the notification fd to the parent.
		 */
		if (send_fd(j->filter_socket_fds[0], notification_fd))
			pdie("send_fd() failed");
		...
		/*
		 * minijail_enter() was called in the same process, so the notification fd
		 * can be set directly.
		 */
		if (j->flags.filter_user_notification_fd)
			close_and_reset(&j->filter_user_notification_fd);
		j->filter_user_notification_fd = notification_fd;
...
static void set_seccomp_filter(struct minijail *j)
{
	...
	if (j->flags.seccomp_filter) {
		if (j->flags.seccomp_filter_tsync ||
		    ...
		    j->flags.seccomp_install_user_notification) {
			int filter_flags = 0;
			if (j->flags.seccomp_filter_allow_speculation)
			...
			if (ret < 0) {
				pdie("seccomp(SECCOMP_SET_MODE_FILTER, 0%s%s%s%s) failed",
				     (filter_flags &
					      SECCOMP_FILTER_FLAG_SPEC_ALLOW
					  ? "|SECCOMP_FILTER_FLAG_SPEC_ALLOW"
					  : ""),
			...
			if (j->flags.seccomp_install_user_notification) {
				set_seccomp_notification_fd(j, ret);
			}
		} else {
				 ...
				 struct minijail_run_state *state_out)
{
	...
	if (child_pid) {
		...
		if (j->flags.filter_socket_fds) {
			...
			j->flags.filter_socket_fds = 0;
			if (ret < 0) {
				pwarn("failed to receive user notification fd");
				kill(j->initpid, SIGKILL);
				return ret;
			}
