				 ...
				 struct minijail_run_status *status_out)
{
	...
	if (child_pid) {
		...
		if (use_preload) {
			/*
			 * Set SIGPIPE handler to SIG_IGN to avoid getting
			 * killed by SIGPIPE if the child process finishes
			 * or closes its end of the pipe prematurely.
			 */
			const sighandler_t old_handler =
			    signal(SIGPIPE, SIG_IGN);
			if (old_handler == SIG_ERR)
				pdie("failed to set SIGPIPE handler");
			...
			if (signal(SIGPIPE, old_handler) == SIG_ERR)
				pdie("failed to reset SIGPIPE handler");
