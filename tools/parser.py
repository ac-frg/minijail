...
Statements have a list of syscalls, and each one of those syscalls have a list
of filters that are evaluated sequentially.
"""
...
class PolicyParser:
    ...
    def _parse_single_constant(self, token):
        if token.type == 'IDENTIFIER':
            ...
            self._parser_state.error('invalid constant', token=token)
        if single_constant >= (1 << self._arch.bits):
            self._parser_state.error('unsigned overflow', token=token)
        if single_constant < 0:
            if single_constant < -(1 << (self._arch.bits - 1)):
                self._parser_state.error('signed underflow', token=token)
            single_constant &= ((1 << self._arch.bits) - 1)
    ...
    def parse_value(self, tokens):
        """Parse constants separated by pipes.
        ...
        - A valid constant expression prefixed with the unary bitwise
          complement operator ~.
        - A series of valid constant expressions separated by pipes.
    ...
    def _parse_syscall_descriptor(self, tokens):
        ...
        if '@' in syscall_descriptor.value:
            ...
            if syscall_namespace_name not in self._arch.syscall_groups:
                self._parser_state.error(
                    'nonexistent syscall group namespace', token=syscall_descriptor)
            syscall_namespace = self._arch.syscall_groups[syscall_namespace_name]
    ...
    def parse_filter_statement(self, tokens):
        ...
        tokens.pop(0)
        filter_expressions = self.parse_filter(tokens)
        if not syscall_descriptors:
            return None
        return ParsedFilterStatement(
            tuple(syscall_descriptors), filter_expressions)
    ...
    def parse_file(self, filename):
        ...
        for filter_statement in filter_statements:
            unconditional_actions = sum(1 for filt in filter_statement.filters
                                        if filt.expression is None)
            if unconditional_actions == 1:
                continue
            if unconditional_actions > 1:
                raise Exception(
                    'Syscall %d already had an unconditional action applied' %
                    filter_statement.syscall)
            assert unconditional_actions == 0
