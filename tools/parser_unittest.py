...
class ParseFilterTests(unittest.TestCase):
    ...
    def test_parse_filter(self):
        ...
        self.assertEqual(
            self.parser.parse_filter(self._tokenize('arg0 == 0; allow')), [
                parser.Filter([[parser.Atom(0, '==', 0)]], bpf.Allow()),
            ])
        self.assertEqual(
            self.parser.parse_filter(self._tokenize('arg0 == 0; trap')), [
                parser.Filter([[parser.Atom(0, '==', 0)]], bpf.Trap()),
            ])
        self.assertEqual(
...
class ParseFilterStatementTests(unittest.TestCase):
    ...
    def test_parse_metadata(self):
        ...
        self.assertEqual(
            self.parser.parse_filter_statement(
                self._tokenize('{read, nonexistent[arch=nonexistent]}: arg0 == 0')),
            parser.ParsedFilterStatement((parser.Syscall('read', 0), ), [
                parser.Filter([[parser.Atom(0, '==', 0)]], bpf.Allow()),
            ]))
