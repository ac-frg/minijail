...
class ParseConstantTests(unittest.TestCase):
    ...
    def test_parse_parenthesized_expressions(self):
        ...
        for expression in bad_expressions:
            with self.assertRaises(compile_seccomp_policy.ParseException):
                self.compiler.parse_constant(expression)
        ...
        for expression in good_expressions:
            self.assertEqual(self.compiler.parse_constant(expression), 3)
    ...
    def test_parse_constant_complements(self):
        ...
        if self.arch.bits == 32:
            self.assertEqual(
                self.compiler.parse_constant(
                    ['~', '0x005AF0FF', '~', '0xFFA50FFF']), 0xFFFFFF00)
            self.assertEqual(
                self.compiler.parse_constant([
                    '0x0F', '|', '~', '(', '0x005AF000', '|', '0x00A50FFF',
                    ')', '|', '0xF0'
                ]), 0xFF0000FF)
        else:
            self.assertEqual(
                self.compiler.parse_constant([
                    '~', '0x00005A5AF0F0FFFF', '|', '~', '0xFFFFA5A50F0FFFFF'
                ]), 0xFFFFFFFFFFFF0000)
            self.assertEqual(
                self.compiler.parse_constant([
                    '0x00FF', '|', '~', '(', '0x00005A5AF0F00000', '|',
                    '0x0000A5A50F0FFFFF', ')', '|', '0xFF00'
                ]), 0xFFFF00000000FFFF)
