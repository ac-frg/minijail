...
*** note
**NOTE**: For Chrome OS developers, we currently recommend running this script
**outside of the chroot**. The python bindings for `auparse` seem to work
better outside (tested with x86_64 Debian). The exact cause is yet unknown.
Otherwise, one known pitfall is seeing strange or `unknown-syscall` syscalls
in your ARM policy. Or possibly in general when your workstation's arch is
different than the policy's arch.
***
