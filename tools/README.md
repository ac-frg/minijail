...
Linux kernel v4.14+ support `SECCOMP_RET_LOG`. This allows minijail to log syscalls
via the `audit` subsystem instead of blocking them. One caveat of this approach is
that `SECCOMP_RET_LOG` does not log syscall arguments for finer grained filtering.
The `audit` subsystem itself has a mechanism to log all syscalls. Though a `SYSCALL`
event is more volumnous than a corresponding `SECCOMP` event.
...
Note that this requires python3 bindings for `auparse` which are generally available
in distro packages named `python3-audit` or `python-audit`.

#### Per-boot setup of audit rules on DUT

Set up `audit` rules and an empty seccomp policy for later use. This can be done in
the `pre-start` section of your upstart conf.
