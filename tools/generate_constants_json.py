...
generate the `constants.json` file with that. LLVM IR files are moderately
...
import re
import sys
from typing import Any, Dict, Mapping, NamedTuple, OrderedDict, TextIO
...
class ParseResults(NamedTuple):
    table_name: str
    table_entries: Mapping[str, str]


def parse_llvm_ir(ir: TextIO) -> ParseResults:
    """Parses a single LLVM IR file."""
    string_constants: OrderedDict[str, str] = collections.OrderedDict()
    table_entries: OrderedDict[str, str] = collections.OrderedDict()
    table_name = ''
    for line in ir:
        ...
        if '@syscall_table' in line or '@constant_table' in line:
            ...
            for entry in _TABLE_ENTRY_RE.findall(line):
                ...
                if not groups:
                    logging.error('Failed to parse table entry %r', entry)
                    continue
...
def main() -> int:
    ...
    json.dump(constants_json, args.output, indent='  ')
    return 0
