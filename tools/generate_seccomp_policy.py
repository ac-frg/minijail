...
class BucketInputFiles(argparse.Action):
    ...
    Populates the following namespace attributes:
...
def parse_args(argv):
    ...
    parser.add_argument('input-logs', nargs='+', action=BucketInputFiles,
    ...
    if opts.audit_logs and not auparse:
        parser.error(f'Python bindings for the audit subsystem were not '
                     f'found.\nPlease install the python3-audit (sometimes '
                     f'python-audit) package for your distro to process audit '
                     f'logs: {opts.audit_logs}')
