...
def parse_args(argv):
    ...
    parser.add_argument('--traces', nargs='*', default=[],
                        metavar=('strace1.txt', 'strace2.txt'),
                        help='The strace logs.')
    parser.add_argument('--audit_logs', nargs='*', type=argparse.FileType('r'),
                        default=[], metavar=('audit.log', 'audit.log.1'),
                        help='The audit.log logs.')
    ...
    if opts.audit_logs and 'auparse' not in sys.modules:
        raise Exception('Python bindings for the audit subsystem were not '
                        'found.\nPlease install the python3-audit (sometimes '
                        'python-audit) package for your distro')

    if not opts.traces and not opts.audit_logs:
        raise Exception('No source files specified. Need at least one of '
                        '--traces or --audit_logs')

    if opts.audit_logs and not opts.audit_comm:
        raise Exception('--audit_comm is required with --audit_logs')
...
def parse_audit_log(audit_log, audit_comm, syscalls, arg_inspection):
    ...
    # Quick sanity check for whether this parses as a valid audit log. The
    # first event should have at least one record.
    if not au.first_record():
        raise ValueError('Unable to parse audit log file %s' % audit_log.name)
