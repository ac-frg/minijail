...
def parse_args(argv):
    ...
    parser.add_argument('--trace', dest='traces', default=[], action='append',
                        type=str, metavar='strace.txt',
                        help='One strace log. Repeat for each strace log input')
    parser.add_argument('--audit-log', dest='audit_logs', default=[],
                        action='append', type=argparse.FileType('r'),
                        metavar='audit.log',
                        help='One audit log. Repeat for each audit log input')
    ...
    try:
        if opts.audit_logs and not auparse:
            raise SystemExit('Python bindings for the audit subsystem were not '
                             'found.\nPlease install the python3-audit (sometimes '
                             'python-audit) package for your distro')

        if not opts.traces and not opts.audit_logs:
            raise SystemExit('No source files specified. Need at least one of '
                             '--trace or --audit-log')

        if opts.audit_logs and not opts.audit_comm:
            raise SystemExit('--audit-comm is required with --audit-logs')
    except SystemExit:
        parser.print_help()
        print('\n')
        raise
