  ...
  if (d) {
    while ((p = readdir(d)) != nullptr) {
      if (!strcmp(p->d_name, ".") || !strcmp(p->d_name, ".."))
        continue;
      std::string child = path + "/" + p->d_name;

      struct stat st;
      if (stat(child.c_str(), &st) == -1) {
        fprintf(stderr, "stat(%s): %m", child.c_str());
        success = false;
        continue;
      }
      if (S_ISDIR(st.st_mode)) {
        success &= rmdir_recursive(child);
      } else {
        if (unlink(child.c_str()) == -1) {
          fprintf(stderr, "stat(%s): %m", child.c_str());
          success = false;
        }
      }
    ...
    fprintf(stderr, "opendir(%s): %m", path.c_str());
    success = false;
  }

  if (rmdir(path.c_str()) == -1) {
    fprintf(stderr, "rmdir(%s): %m", path.c_str());
    success = false;
  }
  return success;
}
