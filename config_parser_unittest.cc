...
{
	auto *entry =
	    (struct config_entry *)calloc(1, sizeof(struct config_entry));
	...
	for (const auto conf_line : valid_conf_lines) {
		ASSERT_TRUE(parse_config_line(conf_line.c_str(), entry));
		free(entry->key);
		free(entry->value);
	}
	free(entry);
...
TEST(ParsingConfigTest, invalid_config_line)
{
	...
	for (const auto conf_line : valid_conf_lines) {
		ASSERT_FALSE(parse_config_line(conf_line.c_str(), entry));
	}
	free(entry);
}
...
TEST_F(ConfigFileTest, malformed)
{
	...
	bool res = parse_config_file(config_file, list_);
	fclose(config_file);
